...
roadsmth = ndimage.gaussian_filter1d(pathval, 200)
...
for i in progressbar(range(len(rL))):
    ...
    fade = 1-clamped_scaled*clip_scale
...
mat_blur = gaussian_filter(mat,sigma=3)
# mat = np.where(rmask>0,mat_blur,mat)
mix_mask = np.maximum(fmask,rmask)
mat = mat*mix_mask + mat_blur*(1-mix_mask)

# Write Terrain
imageio.imwrite('terrainOut_curveRoad4.tif',mat.astype(np.float32))
